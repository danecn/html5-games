...
var MMan = Class.extend({
  ...
  update: function() {
    ...
    if (Key.isDown(Key.RIGHT)) {
      this.x += 2;
      this.state = PlayerState.RunningRight;
    } else if (Key.isDown(Key.LEFT)) {
      this.x -= 2;
      this.state = PlayerState.RunningLeft;
    } else {
  ...
  draw: function() {
    if (this.state === PlayerState.Idle) {
      this.drawIdle();
    } else if (this.state === PlayerState.RunningRight) {
      this.drawRightRun();
    } else if (this.state === PlayerState.RunningLeft) {
      this.drawLeftRun();
    }
  }
});
